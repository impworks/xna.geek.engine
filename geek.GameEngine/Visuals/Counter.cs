...
namespace geek.GameEngine.Visuals
{
	...
	/// </summary>
	public class Counter : ObjectGroup
	{
		#region Constructor

		public Counter(Func<VisualObjectBase> generator, Func<int> binding)
		{
			_Generator = generator;
			_Binding = binding;
			_Objects = new List<VisualObjectBase>();

			BindTransparency = true;
		...
		/// </summary>
		private readonly List<VisualObjectBase> _Objects;
		...
		{
			if (_Binding() != _Objects.Count)
				Update();

			if(_Objects.Count == 0)
				return new Rectangle();

			var first = _Objects[0].GetBoundingBox(absolute);
			var last = _Objects[_Objects.Count - 1].GetBoundingBox(absolute);
